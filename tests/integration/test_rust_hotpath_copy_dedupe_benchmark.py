...
def _make_payload(block_size: int, block_count: int) -> bytes:
    return os.urandom(block_size * block_count)
...
def _make_mixed_destination(source_payload: bytes, block_size: int) -> bytes:
    ...
    for block_index in range(0, total_blocks, 3):
        start = block_index * block_size
        end = min(start + block_size, len(buf))
        fill_byte = (block_index * 19 + 37) % 251
...
def main() -> None:
    ...
    dir_path = f"/copy-dedupe-bench-{suffix}"
    ...
    try:
        ...
        if py_result["read_back"] != source_payload:
            raise AssertionError("copy-dedupe/python payload mismatch")
        if rs_result["read_back"] != source_payload:
            raise AssertionError("copy-dedupe/rust payload mismatch")
