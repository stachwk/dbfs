...
- treat the current `bulk_write` vs `metadata_heavy` large-copy comparison as a baseline and continue the Rust POC on the write/copy hot path, with the planner, changed-copy dedupe, changed-run packing, and persist padding enabled by default while Python stays as the fallback, instead of more Python tuning
