...
- treat the current `bulk_write` vs `metadata_heavy` large-copy comparison as a baseline and continue the Rust POC on the write/copy hot path, starting with planner, changed-copy dedupe, and changed-run packing, with the helpers available as runtime opt-ins, instead of more Python tuning
