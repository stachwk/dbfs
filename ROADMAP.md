...
- treat the current `bulk_write` vs `metadata_heavy` large-copy comparison as a baseline and continue the Rust POC on the write/copy hot path, starting with planner and changed-run packing, instead of more Python tuning
