...
- Treat the current copy profile comparison as a frozen baseline; continue the narrow Rust POC for the write/copy hot path, starting with planner and changed-run packing, with the packer available as a runtime opt-in, rather than another round of Python tuning.
