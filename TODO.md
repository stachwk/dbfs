...
- Treat the current copy profile comparison as a frozen baseline; continue the narrow Rust POC for the write/copy hot path, starting with planner, changed-copy dedupe, and changed-run packing, with the helpers available as runtime opt-ins, rather than another round of Python tuning.
