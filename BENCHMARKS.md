...
- The current `bulk_write` vs `metadata_heavy` large-copy comparison is the baseline for profile selection; the Rust POC on the write/copy hot path now lives in `rust_hotpath/` and covers the copy planner plus changed-run packing.
