...
- The current write-path boundary in Rust now includes the first repository-side namespace lookups, hardlink/symlink/create/mknod planning, and logical resize planning for `truncate()` / `fallocate()`; Python still owns orchestration, cache invalidation, and journal append/commit glue.
