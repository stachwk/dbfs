...
- Rustowy hot-path POC jest teraz domyślnie włączony dla planera, changed-copy dedupe, changed-run packing i paddingu bloków. Najnowszy benchmark changed-copy nie pokazał na tym hoście wyraźnego end-to-end zysku, więc fallback w Pythonie nadal zostaje do porównań i ewentualnego wyłączenia.
